    ...
    <form action="" id="budget-form">
        ...
        <?php if (!isset($id)) : ?>
            <div class="form-group">
                ...
                <select name="category_id" id="category_id" class="custom-select select2" required>
                    <option value=""></option>
                    <?php
                    $qry = $conn->query("SELECT * FROM `categories` WHERE user_id = $user_id order by category asc");
        ...
        <?php else : ?>
            <div class="form-group">
                ...
                <?php
                $qry = $conn->query("SELECT * FROM `categories` where id = '{$category_id}' AND user_id = $user_id  ");
